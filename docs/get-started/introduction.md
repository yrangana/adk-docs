...
The Agent Development Kit (ADK) is designed to empower developers
...
* **Agent:** The fundamental worker unit designed for specific tasks. Agents can
  use language models (`LlmAgent`) for complex reasoning or act as controllers
  (`SequentialAgent`, `ParallelAgent`) to manage workflows.
...
Agent Development Kit offers several key advantages for developers building
...
3. **Flexible Orchestration:** Define complex agent workflows using built-in
   container agents (`SequentialAgent`, `ParallelAgent`, `LoopAgent`) alongside
...
6. **Built-in Agent Evaluation:** Assess agent performance systematically. The
   ...
   evaluations locally (via CLI or the developer UI) to measure quality and
   guide improvements.
